	...
	<view>
		<view class="info-box">
			<view class="user-item">
				...
				<u-button v-show="postDetail.is_follow" size="mini" style="float:right;font-size: 14px;" @click="cancelFollow">已关注</u-button>
				<u-button v-show="!postDetail.is_follow" size="mini" style="float:right;font-size: 14px;" @click="follow">关注</u-button>
			</view>
			<view class="icon">
			...
			<view v-if="postDetail.type === 4" class="vote-box">
				...
				<view v-else class="expire-time">截止：{{ postDetail.vote_info.expire_time | date('yyyy年mm月dd日hh时MM分') }}</view>
				<view
					class="vote-item"
					@click="castVote(index2, postDetail.vote_info.type)"
					:class="{ active: item2.checked }"
					v-for="(item2, index2) in postDetail.vote_info.options"
					:key="index2"
				>
					<text v-if="postDetail.is_vote_result || isVoteExpire">{{ item2.content }}<text style="color: #999;margin-left: 20rpx;">({{ item2.ticket_num }}票)</text></text>
					<text v-else="postDetail.is_vote_result">{{ item2.content }}</text>
				</view>
				<q-button v-if="!postDetail.is_vote_result && isVoteExpire === false" @click="voteSubmit">投票</q-button>
			</view>
			<!-- 圈子信息 -->
			<navigator class="topic-info" :url="'/pages/topic/detail?id=' + postDetail.topic_id">
				...
				<view class="center">
					...
					<view class="count-txt">{{ postDetail.topic_info.user_count }}人加圈 | {{ postDetail.topic_info.post_count }}篇内容</view>
				</view>
				<view class="right">
					<text>去看看</text>
		...
		<view class="comment-box">
			...
			<view style="margin-bottom: 40rpx;" v-for="(item, index) in commentList" :key="index">
				...
				>
					...
					<view class="reply">
						<text>回复</text>
						<navigator :url="'/pages/user/home?uid=' + item2.to_user.uid" hover-class="none" class="name">{{ item2.to_user.username }}</navigator>
...
export default {
	...
	methods: {
		...
		async shareCanvas() {
			...
			this.drawText(context, this.postDetail['content'], 20, 260, 20, 260, 5);

			// 来源
			context.setFillStyle('#181818');
			context.setFontSize(14);
			context.fillText('DeadLine - 发现趣味相投的人', 60, 430, 300);

			// 圈子二维码
			let qrCode = await this.getQrCode();
			let tempQrCode = await this.downloadFile(qrCode);
			context.drawImage(tempQrCode, 105, 300, 100, 100);
			context.draw();
			uni.hideLoading();
		},
		/**
